Vue.component('filter-form', {
	template: `
		<form class="filter-form">
			<div class="grid-container">
				<div>
					<p>
						<label for="searchCode">Code: </label>
						<input id="searchCode" v-model="searchCode"></input>
					</p>
					<p>
						<label for="stroke">筆劃：</label>
						<select id="stroke" v-model.number="stroke">
							<option></option>
							<option v-for="n in 26"	:key="n" :value="n">
								{{n}}
							</option>
						</select>
					</p>
					<p>
						<label for="nameLength">歌名字數：</label>
						<select id="nameLength" v-model.number="nameLength">
							<option></option>
							<option v-for="n in 18"	:key="n" :value="n">
								{{n}}
							</option>
						</select>
					</p>
					<p>
						<label for="wordContains">包含關鍵詞：</label>
						<input id="wordContains" v-model="wordContains"></input>
					</p>
				</div>
				<p>
					<label for="key">調：</label>
					<select id="key" v-model="key">
						<option></option>
						<option value="C">C</option>
						<option value="Cm">Cm</option>
						<option value="D">D</option>
						<option value="Dm">Dm</option>
						<option value="E">E</option>
						<option value="Eb">Eb</option>
						<option value="Em">Em</option>
						<option value="F">F</option>
						<option value="G">G</option>
						<option value="Gm">Gm</option>
						<option value="A">A</option>
						<option value="Ab">Ab</option>
						<option value="Am">Am</option>
						<option value="Bb">Bb</option>
						<option value="Bbm">Bbm</option>
						<option value="Bm">Bm</option>
					</select>
				</p>
			</div>
			<p>
				<button type="button" class="button" @click="onSubmit">搜尋</button>
				<button type="button" class="button" @click="onShowAll">顯示全部</button>
			</p>
		</form>
	`,
	data() {
		return {
			stroke: null,
			nameLength: null,
			searchCode: null,
			wordContains: null,
			key: null,
		}
	},
	methods: {
		onSubmit() {
			let searchTerms = {
				stroke: this.stroke,
				nameLength: this.nameLength,
				searchCode: this.searchCode,
				wordContains: this.wordContains,
				key: this.key,
			}
			
			this.$emit('search-terms-entered', searchTerms)
		},
		onShowAll() {
			let searchTerms = {
				stroke: null,
				nameLength: null,
				searchCode: null,
				wordContains: null,
				key: null,
			}
			
			this.stroke = null
			this.nameLength = null
			this.searchCode = null
			this.wordContains = null
			this.key = null
			
			this.$emit('search-terms-entered', searchTerms)
		},
	}
})

Vue.component('filtered-list', {
	props: {
		filteredSongs: {
		}
	},
	template: `
		<table>
			<tr>
				<th style="min-width:80px">Code</th>
				<th style="min-width:250px">歌名</th>
				<th style="width:60%">第一句</th>
				<th>調</th>
				<th>Remark</th>
				<th style="min-width:70px">歌名字數</th>
			</tr>
			<tr v-for="song in filteredResults" :key="song.code">
				<td>{{ song.code }}</td>
				<td>{{ song.name }}</td>
				<td>{{ song.firstLine }}</td>
				<td>{{ song.key }}</td>
				<td>{{ song.remark }}</td>
				<td>{{ song.name.length }}</td>
			</tr>
		</table>
	`,
	data() {
		return {
		}
	},
	methods: {
	},
	computed: {
		filteredResults() {
			return this.filteredSongs
		}
	}
	
})

var app = new Vue({
	el: '#app',
	mounted:function(){
		let searchTerms = {
			stroke: null,
			nameLength: null,
			searchCode: null,
			wordContains: null,
		}
		
		this.search(searchTerms) //search will execute at pageload
	},
	data: {
		worship: '迦琳基督教會祟拜詩歌',
		image: './images/Cross.jpg',
		filteredSongs: [],
		songs: [
			{
				code: '1-Y0101',
				name: '一件禮物',
				firstLine: '有一件禮物你收到沒有',
				remark: '',
				stroke: 1,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '1-Y0102',
				name: '一顆不變心',
				firstLine: '牽我心牽我手',
				remark: 'BY TTK',
				stroke: 1,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '1-Y0103',
				name: '一生不枉過',
				firstLine: '為何忘掉那日',
				remark: '',
				stroke: 1,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '1-Y0104',
				name: '一生愛你',
				firstLine: '親愛的寶貴耶穌',
				remark: '',
				stroke: 1,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '1-Y0105',
				name: '一首讚美的詩歌',
				firstLine: '讓那讚美的聲音',
				remark: '',
				stroke: 1,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '1-Y0106',
				name: '一生不會捨棄你',
				firstLine: '主是何等的重要',
				remark: '',
				stroke: 1,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '1-Y0107',
				name: '一粒麥子',
				firstLine: '一粒麥子它若不落在地裡死了',
				remark: '',
				stroke: 1,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '2-S0101',
				name: '十字架永是我的榮耀',
				firstLine: '主我來就十字架',
				remark: '副歌：十字架十字架',
				stroke: 2,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '2-S0102',
				name: '十架背後',
				firstLine: '凝望昂十架的背後',
				remark: 'BY TTK',
				stroke: 2,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '2-S0103',
				name: '十架的冠冕',
				firstLine: '路是難走',
				remark: '',
				stroke: 2,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '2-S0104',
				name: '十字架',
				firstLine: '',
				remark: '',
				stroke: 2,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '2-S0105',
				name: '十架的愛',
				firstLine: '謙卑的君王榮耀的君王',
				remark: '',
				stroke: 2,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '2-Y0101',
				name: '人算什麼',
				firstLine: '人算什麼你竟看顧',
				remark: '',
				stroke: 2,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '2-Y0102',
				name: '人算甚麼',
				firstLine: '耶和華我們的主',
				remark: '',
				stroke: 2,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '3-F0101',
				name: '凡事都能做',
				firstLine: '信是未曾看見',
				remark: '',
				stroke: 3,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '3-F0102',
				name: '凡事謙虛',
				firstLine: '凡事謙虛溫柔忍耐',
				remark: '',
				stroke: 3,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '3-G0101',
				name: '工人的禱告',
				firstLine: '獻上一生為昂而活',
				remark: '',
				stroke: 3,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '3-S0101',
				name: '三一神',
				firstLine: '三一神我要深愛你',
				remark: '',
				stroke: 3,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '3-S0201',
				name: '上帝聽禱告',
				firstLine: '誰了解此時我心正淚流像火燙',
				remark: '',
				stroke: 3,
				key: 'Gm',
				link: '',
				lyrics: ''
			},
			{
				code: '3-X0101',
				name: '小寶貝',
				firstLine: '耶穌一見你寶貝就歡笑',
				remark: '',
				stroke: 3,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '4-B0101',
				name: '不要只風聞有你',
				firstLine: '過去曾風聞有你',
				remark: '',
				stroke: 4,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '4-B0102',
				name: '不再是我乃是基督',
				firstLine: '現在活著的不再是我',
				remark: '',
				stroke: 4,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '4-J0101',
				name: '日日夜夜',
				firstLine: '耶和華昂大榮耀降臨',
				remark: '',
				stroke: 4,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '4-S0101',
				name: '心靈的迴響',
				firstLine: '天空裡散聚雲片片',
				remark: '',
				stroke: 4,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '4-T0101',
				name: '天糧',
				firstLine: '望盡是芬芳花草滿山',
				remark: '',
				stroke: 4,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '4-T0102',
				name: '天黑黑',
				firstLine: '哪管天黑黑不用去驚怕',
				remark: '',
				stroke: 4,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '4-T0103',
				name: '天離地有多麼高',
				firstLine: '天離地有多麼高 東離西有多麼遠',
				remark: '',
				stroke: 4,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '4-T0104',
				name: '天使高唱在高天',
				firstLine: '天使歌唱在高天',
				remark: ' 聖誕用',
				stroke: 4,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '4-T0105',
				name: '天天歌唱',
				firstLine: '天天歌唱頌讚神慈恩',
				remark: '',
				stroke: 4,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '4-T0106',
				name: '天地讚美',
				firstLine: '地與海奏出歌韻',
				remark: '',
				stroke: 4,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '4-Z0101',
				name: '中國早晨五點鍾',
				firstLine: '中國早晨五點鍾',
				remark: '',
				stroke: 4,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '5-C0101',
				name: '充滿我',
				firstLine: '我的心惟有你我的靈渴慕你',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-D0101',
				name: '叮噹叮',
				firstLine: '叮噹叮，叮噹叮，歌聲與鐘聲',
				remark: ' 聖誕用',
				stroke: 5,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '5-D0201',
				name: '打開天窗',
				firstLine: '願我我的禱告如香陳列在你面前',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-F0101',
				name: '付出的愛',
				firstLine: '每當我想起昂寶貴十架',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-G0101',
				name: '古老的故事',
				firstLine: '聖誕鐘聲響遍城',
				remark: '',
				stroke: 5,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '5-G0201',
				name: '叫我抬起頭的神',
				firstLine: '耶和華是我的主',
				remark: '',
				stroke: 5,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0101',
				name: '主呀我只想要你',
				firstLine: '主呀我只想要你',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0102',
				name: '主我們需要昂',
				firstLine: '主，罪人需要昂',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0103',
				name: '主我高舉你的名',
				firstLine: '主我高舉你的名',
				remark: '',
				stroke: 5,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0104',
				name: '主我敬拜昂',
				firstLine: '主我敬拜昂',
				remark: '',
				stroke: 5,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0105',
				name: '主的慈愛',
				firstLine: '主昂的慈愛比生命更好',
				remark: '',
				stroke: 5,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0106',
				name: '主的愛',
				firstLine: '人若信靠救主 主旳愛赦免了罪人',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0107',
				name: '主是平安',
				firstLine: '主是平安 祂已拆祢中間隔斷的牆',
				remark: '',
				stroke: 5,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0108',
				name: '主是安息港',
				firstLine: '心渴慕恩主現臨到我',
				remark: '',
				stroke: 5,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0109',
				name: '主是愛',
				firstLine: '主是愛主是愛主愛永遠不更改',
				remark: '',
				stroke: 5,
				key: 'Cm',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0110',
				name: '主為我開路',
				firstLine: '主為我開路',
				remark: '',
				stroke: 5,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0111',
				name: '主耶和華的喜樂是我的力量',
				firstLine: '主耶和華的喜樂是我的力量',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0112',
				name: '主耶穌真愛我',
				firstLine: '青草地上滿牛羊',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0113',
				name: '主耶穌愛我',
				firstLine: '耶穌愛我真可信',
				remark: '',
				stroke: 5,
				key: 'Eb',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0114',
				name: '主恩夠用',
				firstLine: '昂的恩典每天夠我用',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0115',
				name: '主恩頌',
				firstLine: '神救恩廣賜天下',
				remark: '',
				stroke: 5,
				key: 'Am',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0116',
				name: '主啊你的溫柔',
				firstLine: '主啊你的溫柔',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0117',
				name: '主啊你創造我',
				firstLine: '主啊你創造我',
				remark: '',
				stroke: 5,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0118',
				name: '主啊我心愛你',
				firstLine: '我眼流淚 我心破碎',
				remark: '',
				stroke: 5,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0119',
				name: '主啊我讚美你',
				firstLine: '主啊我讚美你 因為你揀選了我',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0120',
				name: '主啊賜下靈火',
				firstLine: '主啊賜下靈火',
				remark: '',
				stroke: 5,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0121',
				name: '主給我子女名份',
				firstLine: '耶和華是創造天與海',
				remark: '',
				stroke: 5,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0122',
				name: '主愛不變',
				firstLine: '主的愛從來沒有改變',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0123',
				name: '主愛必堅固',
				firstLine: '神愛滔滔活水匆匆湧流',
				remark: '',
				stroke: 5,
				key: 'Eb',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0124',
				name: '主愛同在',
				firstLine: '來同頌 頌主愛',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0125',
				name: '主當我向昂跪下的時候',
				firstLine: '主當我向昂跪下的時候',
				remark: '',
				stroke: 5,
				key: 'Bbm',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0126',
				name: '主昂是我力量',
				firstLine: '我的詩歌 我的拯救',
				remark: '',
				stroke: 5,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0127',
				name: '主的呼喚',
				firstLine: '我可以差遣誰',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0128',
				name: '主的恩典',
				firstLine: '夜深天色已漸迷濛',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0129',
				name: '主我跟昂走',
				firstLine: '主我跟昂走',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0130',
				name: '主你是榮耀的主',
				firstLine: '你是榮耀的君王',
				remark: '',
				stroke: 5,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0131',
				name: '主神啊諸天被創造',
				firstLine: '主神啊!諸天被創造',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0132',
				name: '主啊昂配得受讚美',
				firstLine: '主啊昂配得受讚美',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0133',
				name: '主信實無變',
				firstLine: '慈愛主信實無變',
				remark: '',
				stroke: 5,
				key: 'Eb',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0134',
				name: '主啊教我們禱告',
				firstLine: '我們在天上的阿爸父',
				remark: '',
				stroke: 5,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0135',
				name: '主的喜樂是我力量',
				firstLine: '主的喜樂是我力量',
				remark: '',
				stroke: 5,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0136',
				name: '主我心尋找昂',
				firstLine: '主我心尋找昂',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0137',
				name: '主的喜樂是我的力量',
				firstLine: '耶和華的喜樂是我們的力量',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0138',
				name: '主你永遠與我同在',
				firstLine: '危難臨到我信靠你深知你必領我渡過',
				remark: '',
				stroke: 5,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0139',
				name: '主禱文(請教導我們禱告)',
				firstLine: '父你的名是多麼美榮耀遍全地',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0140',
				name: '主恩實在更多',
				firstLine: '我真希奇救主耶穌',
				remark: '',
				stroke: 5,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0141',
				name: '主禱文',
				firstLine: '我們在天上的父願人都尊你的名為聖',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0201',
				name: '只因愛',
				firstLine: '昂的愛我怎猜透 捨尊降貴',
				remark: '',
				stroke: 5,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0202',
				name: '只有昂永恆的主',
				firstLine: '誰知心痛苦 誰解心裡憂',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-J0203',
				name: '只向祂讚美',
				firstLine: '我的心一生只向祂讚美',
				remark: '',
				stroke: 5,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '5-L0101',
				name: '立志擺上',
				firstLine: '曾擺上傾出大愛',
				remark: '',
				stroke: 5,
				key: 'Eb',
				link: '',
				lyrics: ''
			},
			{
				code: '5-M0101',
				name: '末日迫近了',
				firstLine: '飢荒越來越厲害了',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-S0101',
				name: '生命在於昂',
				firstLine: '我生命在於昂',
				remark: '',
				stroke: 5,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '5-S0102',
				name: '生命有價',
				firstLine: '盡快將憂愁眼睛憂愁面孔憂愁內心拋棄吧',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-S0103',
				name: '生命活祭',
				firstLine: '我心充滿讚美 衷心感謝我主耶穌',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-S0201',
				name: '世界變',
				firstLine: '天空的飛鳥不耕作也不收',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-W0101',
				name: '永不能止息',
				firstLine: '主你使我得蒙恩',
				remark: '',
				stroke: 5,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '5-W0102',
				name: '永不渴',
				firstLine: '人若渴了可到我這裡',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-W0103',
				name: '永遠尊貴',
				firstLine: '耶穌真神羔羊',
				remark: '',
				stroke: 5,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '5-W0201',
				name: '禾場',
				firstLine: '禾場上千百萬人流浪',
				remark: '',
				stroke: 5,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '5-Y0101',
				name: '以色列的聖者',
				firstLine: '以色列的聖者 為我犧牲自己',
				remark: '',
				stroke: 5,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '5-Y0102',
				name: '以馬內利',
				firstLine: 'EMMANUEL 神與你同在',
				remark: '',
				stroke: 5,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '6-B0101',
				name: '冰天的太陽',
				firstLine: '人間可有熱愛如艷陽',
				remark: '',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-C0101',
				name: '充滿我 親愛聖靈',
				firstLine: '充滿我 親愛聖靈',
				remark: '',
				stroke: 6,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '6-D0101',
				name: '多愛',
				firstLine: '多愛 多能力',
				remark: '',
				stroke: 6,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '6-D0102',
				name: '多謝耶穌',
				firstLine: '',
				remark: '',
				stroke: 6,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '6-H0101',
				name: '向主呼喊',
				firstLine: '耶穌 我救主 你至聖有誰能比',
				remark: '',
				stroke: 6,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '6-H0102',
				name: '向至高者讚美',
				firstLine: '同心皆向至高者讚美',
				remark: '神啊我們的偉大磐石',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-H0103',
				name: '向君王歡呼',
				firstLine: '向君王歡呼 來歌唱讚美祂',
				remark: '',
				stroke: 6,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '6-H0104',
				name: '向前進把福音傳揚',
				firstLine: '向前進把福音傳揚',
				remark: '',
				stroke: 6,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '6-J0101',
				name: '至尊的愛',
				firstLine: '耶穌愛你 耶穌愛你 地老天荒也不倦',
				remark: '',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-J0201',
				name: '如鹿切慕溪水',
				firstLine: '神啊我心切慕昂如鹿切慕溪水',
				remark: '',
				stroke: 6,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '6-J0202',
				name: '如果你想知道',
				firstLine: '如果你想知道愛在哪裡, 愛就在你我的周圍',
				remark: '',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-J0203',
				name: '如此認識我',
				firstLine: '誰像你如此認識我',
				remark: '',
				stroke: 6,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '6-O0101',
				name: '安穩在主裡',
				firstLine: '恩主，賜我安息',
				remark: '',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-O0102',
				name: '安靜',
				firstLine: '藏我在翅膀蔭下',
				remark: '',
				stroke: 6,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '6-O0103',
				name: '安靜',
				firstLine: '安靜讓平安入我心',
				remark: '讚美之泉VERSION',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Q0101',
				name: '全地宣告',
				firstLine: '全地宣告 榮耀復活救世主',
				remark: '',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Q0102',
				name: '全能父上帝',
				firstLine: '全能父上帝晝夜掌管天地萬邦',
				remark: '',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Q0103',
				name: '全然向你',
				firstLine: '當讚美的旋律響起',
				remark: '',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Q0104',
				name: '全新的昂',
				firstLine: '你說陰天代表你的心情',
				remark: '',
				stroke: 6,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Q0105',
				name: '全地至高是我主',
				firstLine: '願靈內甦醒',
				remark: '',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Q0106',
				name: '全因為昂',
				firstLine: '常言道：人生像演戲',
				remark: '',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Q0107',
				name: '全能上帝是我的主我的神',
				firstLine: '全能上帝是我的主我的神',
				remark: '',
				stroke: 6,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '6-S0101',
				name: '先求祂的國和祂的義',
				firstLine: '你們要先求祂的國',
				remark: '',
				stroke: 6,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '6-T0101',
				name: '同在的神 以馬內利',
				firstLine: '道就在萬有首先',
				remark: '',
				stroke: 6,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '6-T0102',
				name: '同路人',
				firstLine: '只因為我們都是同路人',
				remark: '',
				stroke: 6,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '6-T0103',
				name: '同頌慶主恩典',
				firstLine: '同頌慶主恩典',
				remark: '',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-T0104',
				name: '同讚美耶穌得榮耀',
				firstLine: '是昂寧下降凡塵',
				remark: '',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-W0101',
				name: '回到這個家',
				firstLine: '假使結局已不可改究竟怎可看開',
				remark: '',
				stroke: 6,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '6-W0102',
				name: '回歸昂身邊',
				firstLine: '你誠然擔當憂患',
				remark: '',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-W0103',
				name: '回家',
				firstLine: '親愛耶穌,我感謝昂來到這世界',
				remark: '',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-W0104',
				name: '回家',
				firstLine: '步伐是否感覺疲倦了',
				remark: '',
				stroke: 6,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Y0101',
				name: '因主的名',
				firstLine: '造物主 今高唱昂名字',
				remark: '',
				stroke: 6,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Y0102',
				name: '因主看顧',
				firstLine: '大海翻滾山嶺動搖',
				remark: '',
				stroke: 6,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Y0103',
				name: '因那在我裡面的',
				firstLine: '因那在我裡面的比那在世界的更大',
				remark: '',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Y0104',
				name: '因祂活著',
				firstLine: '神差愛子 人稱祂耶穌',
				remark: '',
				stroke: 6,
				key: 'Eb',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Y0105',
				name: '因著信',
				firstLine: '憑信他離別吾珥',
				remark: '',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Y0201',
				name: '有了我的命令又遵守的',
				firstLine: '有了我的命令又遵守的',
				remark: '',
				stroke: 6,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Y0202',
				name: '有你 … 和平之君',
				firstLine: '有你陪我走',
				remark: '',
				stroke: 6,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Y0203',
				name: '有誰能像你',
				firstLine: '有誰能像你 耶穌只有你能感動我心',
				remark: '',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Y0204',
				name: '有昂我就有亮光',
				firstLine: '有昂我就有亮光 有昂我就有方向',
				remark: '',
				stroke: 6,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Y0205',
				name: '有一位神',
				firstLine: '有一位神',
				remark: '',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0101',
				name: '在天上的爸爸',
				firstLine: '在天上的爸爸 願人人都尊你名為聖',
				remark: '',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0102',
				name: '在主前歡樂',
				firstLine: '在主前歡樂 唱哈利路亞',
				remark: '',
				stroke: 6,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0103',
				name: '在主愛中',
				firstLine: '清晨朝陽天空照耀',
				remark: '',
				stroke: 6,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0104',
				name: '在乎您',
				firstLine: '您的慈愛比天高 比海深',
				remark: '',
				stroke: 6,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0105',
				name: '在末後的日子',
				firstLine: '在末後的日子 我要將我的靈',
				remark: '',
				stroke: 6,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0106',
				name: '在昂寶座前',
				firstLine: '在昂寶座前 我獻上我的敬拜',
				remark: '',
				stroke: 6,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0107',
				name: '在昂手中',
				firstLine: '耶和華我的神 昂是我的神',
				remark: '',
				stroke: 6,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0108',
				name: '在耶穌的腳前',
				firstLine: '在耶穌的腳前',
				remark: '',
				stroke: 6,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0109',
				name: '在教會中的榮耀',
				firstLine: '願祂在教會中得榮耀',
				remark: '',
				stroke: 6,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0110',
				name: '在這裡',
				firstLine: '神的子民合一聚集',
				remark: '',
				stroke: 6,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '6-Z0201',
				name: '早晨的聲音',
				firstLine: '耶和華求你留心聽我的言語',
				remark: '',
				stroke: 6,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-A0101',
				name: '阿爸阿爸父',
				firstLine: '阿爸阿爸父',
				remark: '',
				stroke: 7,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '7-B0101',
				name: '兵歌',
				firstLine: '就是我主耶穌令我生活有力',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-B0201',
				name: '把冷漠變成愛',
				firstLine: '你的眼是否被太多美麗的事物迷惑',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-F0101',
				name: '快樂常歌唱',
				firstLine: '莫記憂與愁要快樂常歌唱',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-F0102',
				name: '快上路',
				firstLine: '快上路，天國呼聲漸近',
				remark: '',
				stroke: 7,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '7-G0101',
				name: '君尊祭司來讚美',
				firstLine: '來讚美祂 君尊祭司',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-G0102',
				name: '君王就在這裡',
				firstLine: '聖潔耶穌昂寶座來這裡',
				remark: '',
				stroke: 7,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '7-G0201',
				name: '告別苦惱',
				firstLine: '開心給我開心',
				remark: 'BY TTK',
				stroke: 7,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '7-G0301',
				name: '改寫我一生',
				firstLine: '容我配看昂榮光',
				remark: 'BY TTK',
				stroke: 7,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '7-G0401',
				name: '更新我心意',
				firstLine: '更新我心意 使我能像昂',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-G0402',
				name: '更像昂',
				firstLine: '我渴望更像昂',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-H0101',
				name: '何等恩典',
				firstLine: '以真誠的心降服在昂面前',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-K0101',
				name: '求引領我心',
				firstLine: '求引領我心',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-K0102',
				name: '求主給我一顆心',
				firstLine: '求主給我一顆心',
				remark: '',
				stroke: 7,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '7-K0103',
				name: '求主興起禱告的心',
				firstLine: '在這片乾旱土地',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-K0104',
				name: '求賜復興之火',
				firstLine: '求主賜復興之火',
				remark: '',
				stroke: 7,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '7-K0105',
				name: '求昂將我輕放於心上',
				firstLine: '求昂將我輕放於心上',
				remark: '',
				stroke: 7,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '7-K0106',
				name: '求主充滿我',
				firstLine: '主我來尋求昂的面',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-M0101',
				name: '每個新的早晨',
				firstLine: '新  每個新的早晨',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-M0102',
				name: '每天每刻獻上為你',
				firstLine: '神昂聖潔, 當得尊貴',
				remark: '',
				stroke: 7,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '7-M0103',
				name: '每一天我需要你',
				firstLine: '讓我坦然無懼來到施恩座前',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-M0201',
				name: '沒有昂那有我',
				firstLine: '從來未試過將一生交托為昂真正活過',
				remark: '',
				stroke: 7,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0101',
				name: '你不要害怕',
				firstLine: '你不要害 因為我與你同在',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0102',
				name: '你叫我活得比世人更美好',
				firstLine: '主我所有好處沒有不在你以外',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0103',
				name: '你有多少心裡話',
				firstLine: '你有多少心裡話對主傾訴',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0104',
				name: '你是我神',
				firstLine: '你是我神 我醫治的神',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0105',
				name: '你是良善',
				firstLine: '我一切的好處在主裡面',
				remark: '',
				stroke: 7,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0106',
				name: '你們要嘗嘗主恩的滋味',
				firstLine: '你們要嘗嘗主恩的滋味',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0107',
				name: '你們要讚美耶和華',
				firstLine: '你們要讚美耶和華 在祂的聖所讚美祂',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0108',
				name: '你是我藏身處',
				firstLine: '你是我藏身處',
				remark: '',
				stroke: 7,
				key: 'Am',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0109',
				name: '你愛主麼',
				firstLine: '你說你已經相信耶穌',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0110',
				name: '你以愛今觸摸我心',
				firstLine: '',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0111',
				name: '你許可',
				firstLine: '',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0112',
				name: '你是我的神',
				firstLine: '你是我的神我要切切尋求你',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0113',
				name: '你的恩典夠我用',
				firstLine: '你的恩典夠我用,在我軟弱困苦中',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0114',
				name: '你莫再慨嘆',
				firstLine: '你莫再慨嘆 你別要悲哀',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0115',
				name: '你是我的一切',
				firstLine: '你是我依靠的力量',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0116',
				name: '你愛永不變',
				firstLine: '你流出寶血洗淨我污穢',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0201',
				name: '我要順服',
				firstLine: '耶穌基督是至寶',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0202',
				name: '我需要有你生命中',
				firstLine: '哦主 求你保守我的心我的意念',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0203',
				name: '我知誰管著明天',
				firstLine: '我不知明天的道路,每一天只為主活',
				remark: '',
				stroke: 7,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0204',
				name: '我要歌頌昂聖名',
				firstLine: '我要歌頌昂聖名',
				remark: '',
				stroke: 7,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0205',
				name: '我要敬拜主耶穌直到永遠',
				firstLine: '我要敬拜主耶穌直到永遠',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0206',
				name: '我心緊緊跟隨你',
				firstLine: '每當我軟弱疲乏',
				remark: '',
				stroke: 7,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0207',
				name: '我主耶穌是生命源',
				firstLine: '我主耶穌是生命源 我主耶穌是活水泉',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0208',
				name: '我一生最大的事是認識耶和華',
				firstLine: '我一生最大的事 是認識耶和華',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0209',
				name: '我心',
				firstLine: '我站在至高處',
				remark: 'BY TTK',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0210',
				name: '我心切切渴慕昂',
				firstLine: '我心切切渴慕昂 乾旱疲乏無水之地',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0211',
				name: '我心尊主為大',
				firstLine: '我心尊主為大',
				remark: '',
				stroke: 7,
				key: 'Ab',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0212',
				name: '我以禱告來到昂跟前',
				firstLine: '我以禱告來到昂跟前 我要尋求昂',
				remark: '',
				stroke: 7,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0213',
				name: '我用主的愛真誠愛昂',
				firstLine: '我用主的愛真誠來愛你',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0214',
				name: '我有這喜樂',
				firstLine: '我有這喜樂非世界賜給我',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0215',
				name: '我的心你要稱頌耶和華',
				firstLine: '我的心你要稱頌耶和華 不可忘記祂的恩惠',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0216',
				name: '我的最愛',
				firstLine: '我愛因昂已先愛我',
				remark: '',
				stroke: 7,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0217',
				name: '我的燈需要油',
				firstLine: '我的燈需要油求主常賜下',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0218',
				name: '我宣告',
				firstLine: '新的一天充滿盼望',
				remark: '',
				stroke: 7,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0219',
				name: '我要以讚美為祭',
				firstLine: '我要以讚美為祭進神的家獻給神',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0220',
				name: '我要向高山舉目',
				firstLine: '我要向高山舉目 我的幫助從何而來',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0221',
				name: '我要建立一群大能子民',
				firstLine: '我要建立一群大能子民',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0222',
				name: '我要唱耶和華的大慈愛',
				firstLine: '我要唱耶和華的大慈愛',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0223',
				name: '我們在天上的父神',
				firstLine: '我們在天上的父神',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0224',
				name: '我們在靈裏合一',
				firstLine: '我們在靈裏合一在主裡合為一',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0225',
				name: '我們成為一家人',
				firstLine: '我們成為一家人',
				remark: '',
				stroke: 7,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0226',
				name: '我們是大能的子民',
				firstLine: '我們是大能的子民',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0227',
				name: '我們進入神的殿',
				firstLine: '我們進入神的殿 奉主命聚集來敬拜祂',
				remark: '',
				stroke: 7,
				key: 'Eb',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0228',
				name: '我們愛',
				firstLine: '你和我是天父愛的創造',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0229',
				name: '我留下平安',
				firstLine: '我留下平安給你們',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0230',
				name: '我發現',
				firstLine: '您是王我是民我屈膝',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0231',
				name: '我愛慕你',
				firstLine: '我的心惟獨被你愛吸引',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0232',
				name: '我獻上生命',
				firstLine: '我獻上生命',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0233',
				name: '我要向高山舉目 (讚美之泉版)',
				firstLine: '我要向高山舉目 我的幫助從何而來',
				remark: '中段: 哈利路亞',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0234',
				name: '我願意',
				firstLine: '神呀 是昂喜歡跟我相親',
				remark: '',
				stroke: 7,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0235',
				name: '我力量在我神',
				firstLine: '我的力量',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0236',
				name: '我能夠看見你的聖潔',
				firstLine: '我能夠看見你的聖潔',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0237',
				name: '我在這裡讚美',
				firstLine: '我在這裡讚美',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0238',
				name: '我要全心讚美',
				firstLine: '全心感謝全心讚美進入你的院',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0239',
				name: '我渴望看見',
				firstLine: '我們等候主不住尋求昂面',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0240',
				name: '我要看見',
				firstLine: '求昂降下同在',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0241',
				name: '我願為你去',
				firstLine: '用聖靈與火為我施洗',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0242',
				name: '我們歡迎君王降臨',
				firstLine: '宇宙的中心耶穌',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0243',
				name: '我們呼求',
				firstLine: '主我們同心在昂面前',
				remark: '',
				stroke: 7,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0244',
				name: '我已得自由',
				firstLine: '在創世以前天父祂已經愛我',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0245',
				name: '我今依賴',
				firstLine: '凡我疑惑全交耶穌',
				remark: '',
				stroke: 7,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0246',
				name: '我全心信靠我主',
				firstLine: '我全心信靠我主',
				remark: '',
				stroke: 7,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0247',
				name: '我向你禱告',
				firstLine: '昂是我神是我的依傍',
				remark: '',
				stroke: 7,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '7-N0301',
				name: '那裡有主的聖靈那裡就有自由',
				firstLine: '那裡有主的聖靈那裡就有自由',
				remark: '',
				stroke: 7,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '7-Z0101',
				name: '助我進深',
				firstLine: '進深 進深',
				remark: '',
				stroke: 7,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '7-Z0201',
				name: '足印',
				firstLine: '憶當天的那段舊夢',
				remark: '',
				stroke: 7,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '7-Z0301',
				name: '住在昂裡面',
				firstLine: '在乾旱無水之地我渴慕你',
				remark: '',
				stroke: 7,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '8-F0101',
				name: '奉主耶穌聖名',
				firstLine: '奉主耶穌聖名',
				remark: '',
				stroke: 8,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '8-F0102',
				name: '奉主基督的名禱告',
				firstLine: '求主將我心化做青草地',
				remark: '',
				stroke: 8,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '8-F0201',
				name: '非勢力',
				firstLine: '非勢力 非才能',
				remark: '',
				stroke: 8,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '8-F0301',
				name: '呼求耶穌',
				firstLine: '求主加倍充滿我心求主加倍釋放生命',
				remark: '',
				stroke: 8,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '8-G0101',
				name: '肩並肩踏上天國路',
				firstLine: '肩並肩踏上天國路 怠恩歌聲飄滿路',
				remark: '',
				stroke: 8,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '8-K0101',
				name: '奇妙雙手',
				firstLine: '一雙手為我 將祝福賜下',
				remark: '',
				stroke: 8,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '8-L0101',
				name: '來頌讚',
				firstLine: '來頌讚神您愛不改',
				remark: '',
				stroke: 8,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '8-L0102',
				name: '來高聲唱',
				firstLine: '來高聲唱 來高聲唱',
				remark: '',
				stroke: 8,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '8-L0103',
				name: '來合一啦',
				firstLine: '頌樂妙曲響遍遍',
				remark: '',
				stroke: 8,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '8-L0104',
				name: '來頌讚主生辰',
				firstLine: '來頌讚主生辰',
				remark: '聖誕用',
				stroke: 8,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '8-M0101',
				name: '明白',
				firstLine: '這一生走過無數的路',
				remark: 'BY TTK',
				stroke: 8,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '8-S0101',
				name: '受綁',
				firstLine: '問我怎向主回應',
				remark: '',
				stroke: 8,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '8-S0201',
				name: '所有的榮耀歸於你',
				firstLine: '榮耀歸於你權柄歸於你',
				remark: '',
				stroke: 8,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '8-T0101',
				name: '祂除去我心中的鎖鍊',
				firstLine: '祂除去我心中的鎖鍊釋放我',
				remark: '',
				stroke: 8,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '8-T0102',
				name: '祂的恩典是一生之久',
				firstLine: '',
				remark: '',
				stroke: 8,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '8-T0103',
				name: '祂名是耶穌',
				firstLine: '祂名是耶穌',
				remark: '',
				stroke: 8,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '8-W0101',
				name: '往前跑',
				firstLine: '向前走呀勇往不退後',
				remark: '',
				stroke: 8,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '8-W0201',
				name: '和撒那',
				firstLine: '和撒那',
				remark: '',
				stroke: 8,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '8-Z0101',
				name: '直到主耶穌再來時候',
				firstLine: '直到主耶穌再來時候',
				remark: '',
				stroke: 8,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '9-H0101',
				name: '看見復興',
				firstLine: '我們是你的百姓',
				remark: '',
				stroke: 9,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '9-K0101',
				name: '祈禱使我常得勝',
				firstLine: '祈禱使我常得勝',
				remark: '',
				stroke: 9,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '9-K0201',
				name: '活著為要敬拜昂',
				firstLine: '開我的眼睛，讓我看見你',
				remark: '',
				stroke: 9,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '9-K0202',
				name: '活出愛',
				firstLine: '有一份愛從天而來比山高比海深',
				remark: '',
				stroke: 9,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '9-K0203',
				name: '活著是為我主',
				firstLine: '活著是為我主 有昂是我一生的褔氣',
				remark: '',
				stroke: 9,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '9-M0101',
				name: '美好新生命',
				firstLine: '神造萬有為彰顯美好生命',
				remark: '',
				stroke: 9,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '9-S0101',
				name: '相信有愛就有奇跡',
				firstLine: '黑夜將過去，未來不再遲疑',
				remark: '',
				stroke: 9,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '9-S0102',
				name: '相聚在愛裡',
				firstLine: '愛使我們相聚一起',
				remark: '',
				stroke: 9,
				key: 'Ab',
				link: '',
				lyrics: ''
			},
			{
				code: '9-S0201',
				name: '信心的等待',
				firstLine: '明天怎麼你還是不知道',
				remark: '',
				stroke: 9,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '9-W0101',
				name: '活著就是基督',
				firstLine: '活著就是基督 活著就是基督',
				remark: '',
				stroke: 9,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '9-W0201',
				name: '為主歌唱',
				firstLine: '為主歌唱 頌讚聲響四方',
				remark: '',
				stroke: 9,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '9-W0202',
				name: '為祂試愛',
				firstLine: '我試過去深愛他 也試過去憎惡他',
				remark: '',
				stroke: 9,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '9-W0203',
				name: '為愛而生',
				firstLine: '有一個夢想在我心頭',
				remark: '',
				stroke: 9,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '9-X0101',
				name: '宣教的中國',
				firstLine: '有一種愛像那夏蟲永長鳴',
				remark: '',
				stroke: 9,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '9-X0201',
				name: '思想他的愛',
				firstLine: '思想他的愛 思想他的良善',
				remark: '',
				stroke: 9,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '9-X0301',
				name: '是昂聲音',
				firstLine: '我呼喚在期待',
				remark: '',
				stroke: 9,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '9-X0302',
				name: '是昂應許',
				firstLine: '壓傷的蘆葦它不折斷',
				remark: '',
				stroke: 9,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0101',
				name: '耶和華以勒',
				firstLine: '綠野碧海生機處處',
				remark: '',
				stroke: 9,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0102',
				name: '耶和華是愛',
				firstLine: '耶和華是愛 讓我安身青草溪水邊',
				remark: '',
				stroke: 9,
				key: 'Eb',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0103',
				name: '耶和華喜樂靈',
				firstLine: '耶和華喜樂靈現在來充滿我',
				remark: '',
				stroke: 9,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0104',
				name: '耶穌，昂的愛已溶化我心',
				firstLine: '耶穌 耶穌 耶穌 昂愛已溶化我心',
				remark: '',
				stroke: 9,
				key: 'Ab',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0105',
				name: '耶穌我愛你',
				firstLine: '耶穌我愛你 我站在你面前',
				remark: '',
				stroke: 9,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0106',
				name: '耶穌基督拯救我生命',
				firstLine: '耶穌基督拯救我生命',
				remark: '',
				stroke: 9,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0107',
				name: '耶穌基督是主',
				firstLine: '耶穌基督是主',
				remark: '',
				stroke: 9,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0108',
				name: '耶穌愛你',
				firstLine: '這世界 有個千年不變道理',
				remark: '',
				stroke: 9,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0109',
				name: '耶穌愛我',
				firstLine: '耶穌愛我 耶穌愛我 耶穌真真愛我',
				remark: '',
				stroke: 9,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0110',
				name: '耶穌謝謝昂的愛',
				firstLine: '當我每感失落沒能力解決是昂令我得安慰',
				remark: '',
				stroke: 9,
				key: 'Eb',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0111',
				name: '耶穌願你為王',
				firstLine: '耶穌願你為王 我們宣揚你是王',
				remark: '',
				stroke: 9,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0112',
				name: '耶穌配得至聖尊榮',
				firstLine: '耶穌配得至聖尊榮',
				remark: '',
				stroke: 9,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0113',
				name: '耶穌的寶血',
				firstLine: '',
				remark: '',
				stroke: 9,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0114',
				name: '耶和華恩年已來到',
				firstLine: '主的靈在我身上',
				remark: '',
				stroke: 9,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0115',
				name: '耶穌基督孤身走窄路',
				firstLine: '',
				remark: '',
				stroke: 9,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0116',
				name: '耶和華昂是我的神',
				firstLine: '耶和華昂是我的神',
				remark: '',
				stroke: 9,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0117',
				name: '耶穌是我生命的主',
				firstLine: '主啊我心切慕主聖靈',
				remark: '',
				stroke: 9,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0118',
				name: '耶穌你是寶貴',
				firstLine: '耶穌你是寶貴',
				remark: '',
				stroke: 9,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0119',
				name: '耶穌領我',
				firstLine: '耶穌領我奔走天路',
				remark: '',
				stroke: 9,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0120',
				name: '耶穌他愛你所以有聖誕節',
				firstLine: '耶穌他愛你所以他願意從天上為你降生',
				remark: '',
				stroke: 9,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0201',
				name: '若你能看見',
				firstLine: '如若你能看見藍天',
				remark: '',
				stroke: 9,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Y0301',
				name: '勇敢走出去',
				firstLine: '我並不完美',
				remark: '',
				stroke: 9,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Z0101',
				name: '珍惜',
				firstLine: '謝謝你天父創造遍世間',
				remark: '',
				stroke: 9,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '9-Z0201',
				name: '祝福你',
				firstLine: '祝福你在主基督裡',
				remark: '',
				stroke: 9,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '10-C0101',
				name: '差遣我',
				firstLine: '主告訴我 如何獻上我的生命',
				remark: '',
				stroke: 10,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '10-C0201',
				name: '除你以外',
				firstLine: '大地頌讚歡唱讚美你名字',
				remark: '',
				stroke: 10,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '10-C0202',
				name: '除昂以外',
				firstLine: '除你以外 在天上我還能有誰',
				remark: '',
				stroke: 10,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '10-G0101',
				name: '剛強壯膽',
				firstLine: '剛強壯膽不要畏懼神現與我們同在',
				remark: '',
				stroke: 10,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '10-G0201',
				name: '高聲宣揚',
				firstLine: '我看見真正路向',
				remark: '',
				stroke: 10,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '10-J0101',
				name: '展開清潔的翅膀',
				firstLine: '主耶和華你已經鑒察了我',
				remark: '',
				stroke: 10,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '10-M0101',
				name: '茫茫宇宙',
				firstLine: '茫茫宇宙有愛心的禮物',
				remark: '',
				stroke: 10,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0101',
				name: '昂的愛',
				firstLine: '讓我生命有著昂的愛',
				remark: 'BY TTK',
				stroke: 10,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0102',
				name: '昂是王',
				firstLine: '神昂極偉大',
				remark: '',
				stroke: 10,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0103',
				name: '昂是君王',
				firstLine: '沒有佩戴冠冕',
				remark: '',
				stroke: 10,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0104',
				name: '昂是我的主',
				firstLine: '昂是我的主 昂是造我的主',
				remark: '',
				stroke: 10,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0105',
				name: '昂是我唯一',
				firstLine: '是甚麼問是甚麼',
				remark: '',
				stroke: 10,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0106',
				name: '昂是神',
				firstLine: '昂創造諸天 昂創造深海',
				remark: '',
				stroke: 10,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0107',
				name: '昂是彌賽亞',
				firstLine: '頌讚主頌讚主高聲歡唱',
				remark: '',
				stroke: 10,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0108',
				name: '昂真偉大',
				firstLine: '我靈歌唱 讚美救主我神',
				remark: '',
				stroke: 10,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0109',
				name: '昂讓我生命改變',
				firstLine: '昂恩典豐富極深厚',
				remark: '',
				stroke: 10,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0110',
				name: '昂是聖潔',
				firstLine: '耶穌昂是何等聖潔美麗',
				remark: '',
				stroke: 10,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0111',
				name: '昂的同在',
				firstLine: '主昂充滿在這地',
				remark: '',
				stroke: 10,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0112',
				name: '昂是配得',
				firstLine: '聖哉聖哉全地唱',
				remark: '',
				stroke: 10,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0113',
				name: '昂以恩典為年歲的冠冕',
				firstLine: '你以恩典為年歲的冠冕',
				remark: '',
				stroke: 10,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0114',
				name: '昂的話',
				firstLine: '我將昂的話語深藏在我心',
				remark: '',
				stroke: 10,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0201',
				name: '哦 聖靈啊',
				firstLine: '哦聖靈啊 求你來潔淨我',
				remark: '',
				stroke: 10,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '10-N0301',
				name: '能不能',
				firstLine: '我屬於昂昂是我永遠的福分',
				remark: '',
				stroke: 10,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '10-P0101',
				name: '配得',
				firstLine: '配得 你是配得 我的主',
				remark: '',
				stroke: 10,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '10-S0101',
				name: '神大恩無限 (神完全的愛)',
				firstLine: '神大恩無限',
				remark: '',
				stroke: 10,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '10-S0102',
				name: '神坐著為王',
				firstLine: '神啊我要一心稱謝昂',
				remark: '',
				stroke: 10,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '10-S0103',
				name: '神是永活全能',
				firstLine: '用鼓樂歌頌 神是永活全能',
				remark: '',
				stroke: 10,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '10-S0104',
				name: '神是我所依',
				firstLine: '祂陪我走過',
				remark: '',
				stroke: 10,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '10-S0105',
				name: '神要開道路',
				firstLine: '神要開道路 在曠野無路之處',
				remark: '',
				stroke: 10,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '10-S0106',
				name: '神啊你在那兒',
				firstLine: '我的天為何總下著雨',
				remark: '',
				stroke: 10,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '10-S0107',
				name: '神啊頌讚歸昂',
				firstLine: '榮耀歸於真神',
				remark: '',
				stroke: 10,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '10-S0108',
				name: '神大愛',
				firstLine: '我唱出神大愛',
				remark: '',
				stroke: 10,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '10-S0109',
				name: '神的靈甦醒我',
				firstLine: '',
				remark: '',
				stroke: 10,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '10-S0110',
				name: '神的帳幕在人間',
				firstLine: '主呀你的國裂天降臨',
				remark: '',
				stroke: 10,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Y0101',
				name: '原來日子多麼美',
				firstLine: '容許我開口說我想見昂',
				remark: '',
				stroke: 10,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Y0102',
				name: '原諒我',
				firstLine: '如果越多的愛是叫人流越多的淚',
				remark: '',
				stroke: 10,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Y0201',
				name: '容我寧靜',
				firstLine: '晨曦之中我尋求神話語',
				remark: '',
				stroke: 10,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Y0301',
				name: '恩典太美麗',
				firstLine: '這天 同聚於主聖殿',
				remark: '',
				stroke: 10,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Y0302',
				name: '恩典夠用',
				firstLine: '我的苦楚 我遭逼迫',
				remark: 'BY TTK',
				stroke: 10,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Y0303',
				name: '恩典夠用',
				firstLine: '當我受特殊試煉',
				remark: '',
				stroke: 10,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Y0304',
				name: '恩典之路',
				firstLine: '你是我的主，引我走正義路',
				remark: '',
				stroke: 10,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Z0101',
				name: '真的有一位神愛你',
				firstLine: '真的有一位神愛你',
				remark: '',
				stroke: 10,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Z0102',
				name: '真的愛你',
				firstLine: '無法可修飾的一對手',
				remark: '母親節用',
				stroke: 10,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Z0103',
				name: '真摯的愛',
				firstLine: '主,都是因為昂的愛',
				remark: '',
				stroke: 10,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Z0201',
				name: '祝福大家',
				firstLine: '祝福大家 一生快樂好福氣',
				remark: '',
				stroke: 10,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '10-Z0202',
				name: '祝福昂',
				firstLine: '',
				remark: '',
				stroke: 10,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '11-C0101',
				name: '唱一首天上的歌',
				firstLine: '生命的河 喜樂的河',
				remark: '',
				stroke: 11,
				key: 'Am',
				link: '',
				lyrics: ''
			},
			{
				code: '11-C0102',
				name: '唱哈利路亞',
				firstLine: '唱哈利路亞讚美主',
				remark: '',
				stroke: 11,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '11-C0201',
				name: '從神而來的真愛',
				firstLine: '從神而來的那些真愛那些真愛',
				remark: '',
				stroke: 11,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '11-C0301',
				name: '將天敞開',
				firstLine: '將天敞開',
				remark: '',
				stroke: 11,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '11-D0101',
				name: '得勝大得勝屬於主',
				firstLine: '得勝大得勝屬於主',
				remark: '',
				stroke: 11,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '11-G0101',
				name: '基督降生',
				firstLine: '聽啊天使讚高聲',
				remark: '聖誕用',
				stroke: 11,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '11-G0102',
				name: '基督生世上為我犧牲洗罪',
				firstLine: '基督生世上為我犧牲洗罪',
				remark: '',
				stroke: 11,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '11-G0103',
				name: '基督降世',
				firstLine: '',
				remark: '聖誕用',
				stroke: 11,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '11-J0101',
				name: '軟弱的我變剛強',
				firstLine: '軟弱的我變剛強',
				remark: '',
				stroke: 11,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '11-M0101',
				name: '問我主恩有幾多',
				firstLine: '問我主恩有幾多 究竟主恩有幾多',
				remark: '',
				stroke: 11,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '11-N0101',
				name: '偶然遇上的驚喜',
				firstLine: '曾經踏遍路途千萬里',
				remark: '',
				stroke: 11,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '11-P0101',
				name: '被救贖的百姓',
				firstLine: '願被救贖的百姓都說',
				remark: '',
				stroke: 11,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '11-S0101',
				name: '常存讚美',
				firstLine: '常存讚美 心中歡欣',
				remark: '',
				stroke: 11,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '11-S0102',
				name: '常常喜樂',
				firstLine: '常常喜樂 向主高歌 不論環境如何',
				remark: '',
				stroke: 11,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '11-S0201',
				name: '深觸我心',
				firstLine: '耶穌誠然擔當 我們的憂患',
				remark: '',
				stroke: 11,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '11-T0101',
				name: '陶造我生命',
				firstLine: '神願你來陶造我',
				remark: '',
				stroke: 11,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '11-W0101',
				name: '唯有耶穌',
				firstLine: '耶穌你能使瞎眼看見',
				remark: '',
				stroke: 11,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '11-W0201',
				name: '惟獨昂(Jesus is Love)',
				firstLine: '今天，盡是滿身傷痕',
				remark: '',
				stroke: 11,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '11-Z0101',
				name: '這世代',
				firstLine: '看哪猶大獅子已得勝',
				remark: '',
				stroke: 11,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '11-Z0102',
				name: '這是完美',
				firstLine: '靠近昂 靠近昂',
				remark: 'BY TTK',
				stroke: 11,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '11-Z0103',
				name: '這一生最美麗的祝福',
				firstLine: '在無數的黑夜裡',
				remark: '',
				stroke: 11,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '11-Z0104',
				name: '這是什麼道理',
				firstLine: '我如此污穢',
				remark: '',
				stroke: 11,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '11-Z0105',
				name: '這是聖潔之地',
				firstLine: '這是聖潔之地 神在這裡',
				remark: '',
				stroke: 11,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '12-C0101',
				name: '創造奇觀',
				firstLine: '事物道理千千',
				remark: '',
				stroke: 12,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '12-C0201',
				name: '尋求神的臉',
				firstLine: '若我的子民 謙卑禱告尋覓著我臉',
				remark: '',
				stroke: 12,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '12-C0202',
				name: '尋找',
				firstLine: '',
				remark: '',
				stroke: 12,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '12-C0203',
				name: '尋著您的那天',
				firstLine: '傳說人生只不過是個夢',
				remark: '',
				stroke: 12,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '12-D0101',
				name: '登山寶訓',
				firstLine: '虛心的人有福了',
				remark: '',
				stroke: 12,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '12-D0102',
				name: '單單只為昂',
				firstLine: '主給我一個夢',
				remark: '',
				stroke: 12,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '12-F0101',
				name: '復興聖潔',
				firstLine: '求昂復興我 心中的聖潔',
				remark: '',
				stroke: 12,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '12-G0101',
				name: '揀選',
				firstLine: '昂十字架的大能',
				remark: '',
				stroke: 12,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '12-H0101',
				name: '喜見心所盼',
				firstLine: '心願能尋路向',
				remark: '',
				stroke: 12,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '12-H0201',
				name: '渴望你的靈',
				firstLine: '心中渴望你的靈',
				remark: '',
				stroke: 12,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '12-H0202',
				name: '渴望常有你',
				firstLine: '在我心中渴望常有你',
				remark: '',
				stroke: 12,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '12-H0301',
				name: '虛心的人有福了',
				firstLine: '虛心的人真是有福 天國是祂們的',
				remark: '',
				stroke: 12,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '12-H0401',
				name: '賀歲歌',
				firstLine: '我地來賀歲 互獻祝福句',
				remark: '',
				stroke: 12,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '12-H0501',
				name: '開口稱頌主',
				firstLine: '我要開口稱頌我的主',
				remark: '',
				stroke: 12,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '12-H0502',
				name: '開開心心高歌',
				firstLine: '開開心心高歌',
				remark: '',
				stroke: 12,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '12-H0503',
				name: '開啟雙眼',
				firstLine: '開你雙眼 看那榮耀的君王',
				remark: '',
				stroke: 12,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '12-M0101',
				name: '無悔一生',
				firstLine: '讓我欣賞你 青春可愛面容',
				remark: '',
				stroke: 12,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '12-N0101',
				name: '雅比斯的禱告',
				firstLine: '',
				remark: '',
				stroke: 12,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '12-N0102',
				name: '雅歌',
				firstLine: '',
				remark: '',
				stroke: 12,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '12-O0101',
				name: '喔!主',
				firstLine: '喔!主, 求你鑒察我',
				remark: '',
				stroke: 12,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '12-P0101',
				name: '普世歡騰',
				firstLine: '普世歡騰 救主下降',
				remark: '',
				stroke: 12,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '12-S0101',
				name: '順服',
				firstLine: '順服昂的揀選 非我選擇要去',
				remark: '',
				stroke: 12,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '12-W0101',
				name: '雲上太陽',
				firstLine: '無論是住在 美麗的高山',
				remark: '',
				stroke: 12,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '12-Y0101',
				name: '揚聲歡呼讚美',
				firstLine: '揚聲歡呼讚美稱頌我的王',
				remark: '',
				stroke: 12,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '12-Z0101',
				name: '最知心的朋友',
				firstLine: '主昂是我最知心的朋友',
				remark: '',
				stroke: 12,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '12-Z0102',
				name: '最美好  盡力愛您',
				firstLine: '最美好曾遇上是您溫柔的接納',
				remark: '',
				stroke: 12,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '12-Z0103',
				name: '最好的福氣',
				firstLine: '願我的心讚頌我的主',
				remark: '',
				stroke: 12,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '12-Z0104',
				name: '最美好的仗',
				firstLine: '過去困於幽暗裡',
				remark: '',
				stroke: 12,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '12-Z0201',
				name: '就在這天',
				firstLine: '讚美我主 讚美我主為我預備',
				remark: '',
				stroke: 12,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '13-D0101',
				name: '當你走到無力',
				firstLine: '當你走到無力繼續下去',
				remark: '',
				stroke: 13,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '13-D0102',
				name: '當聖靈在我的心',
				firstLine: '當聖靈在我的心 我要歌頌主',
				remark: '',
				stroke: 13,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '13-D0103',
				name: '當轉眼仰望耶穌',
				firstLine: '請問你是否困倦煩惱',
				remark: '',
				stroke: 13,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '13-D0104',
				name: '當昂臨近我',
				firstLine: '當昂臨近我 以昂恩手扶持我',
				remark: '',
				stroke: 13,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '13-D0105',
				name: '當信主耶穌',
				firstLine: '',
				remark: '',
				stroke: 13,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '13-G0101',
				name: '感謝主基督',
				firstLine: '是救主的愛將我求贖',
				remark: '',
				stroke: 13,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '13-G0102',
				name: '感謝耶穌',
				firstLine: '感謝耶穌  因昂愛極深',
				remark: '',
				stroke: 13,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '13-G0103',
				name: '感謝天父',
				firstLine: '感謝天父',
				remark: '',
				stroke: 13,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '13-G0201',
				name: '敬拜上帝',
				firstLine: '來敬拜上帝 來讚美上帝',
				remark: '',
				stroke: 13,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '13-G0301',
				name: '跟昂走',
				firstLine: '',
				remark: '',
				stroke: 13,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '13-G0401',
				name: '經由主寶血',
				firstLine: '經由主寶血來到父面前',
				remark: '',
				stroke: 13,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '13-G0501',
				name: '腳步',
				firstLine: '你的腳步帶著我的腳步',
				remark: '',
				stroke: 13,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '13-M0101',
				name: '萬軍之神',
				firstLine: '萬軍之神帶領向前行',
				remark: '',
				stroke: 13,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '13-M0102',
				name: '萬國禱告的殿',
				firstLine: '這稱我名下的殿 必成為萬國禱告的殿',
				remark: '',
				stroke: 13,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '13-M0103',
				name: '萬物的結局近了',
				firstLine: '萬物的結局近了',
				remark: '',
				stroke: 13,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '13-O0101',
				name: '愛 我願意',
				firstLine: '十字架上光茫 溫柔又慈祥',
				remark: '',
				stroke: 13,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '13-O0102',
				name: '愛是不保留',
				firstLine: '常聽說世界愛沒長久',
				remark: '',
				stroke: 13,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '13-O0103',
				name: '愛的真諦',
				firstLine: '',
				remark: '',
				stroke: 13,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '13-O0104',
				name: '愛的迴響',
				firstLine: '基督降生為要拯救我',
				remark: '',
				stroke: 13,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '13-O0105',
				name: '愛中相遇',
				firstLine: '每一天渴望與你在愛中相遇',
				remark: '',
				stroke: 13,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '13-Q0101',
				name: '傳頌',
				firstLine: '願我變做光 光照予四方',
				remark: '',
				stroke: 13,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '13-Q0102',
				name: '傳佳音到',
				firstLine: '傳佳音到誠心禱告同祝福你好',
				remark: '聖誕用',
				stroke: 13,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '13-Q0201',
				name: '慈繩愛索',
				firstLine: '來到昂的寶座前',
				remark: '',
				stroke: 13,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '13-S0101',
				name: '新生王歌',
				firstLine: '聽呀天使高聲唱',
				remark: '聖誕用',
				stroke: 13,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '13-S0102',
				name: '新的異象，新的方向',
				firstLine: '有敬拜的聲音發出',
				remark: '',
				stroke: 13,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '13-S0103',
				name: '新的事將要成就',
				firstLine: '昂是做新事的神',
				remark: '',
				stroke: 13,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '13-X0101',
				name: '聖潔主',
				firstLine: '聖潔主 至聖潔主',
				remark: '',
				stroke: 13,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '13-X0102',
				name: '聖靈請你來',
				firstLine: '聖靈請你來充滿在這地',
				remark: '',
				stroke: 13,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '13-X0103',
				name: '聖靈充滿我',
				firstLine: '聖靈聖靈我感謝你',
				remark: '',
				stroke: 13,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '13-X0201',
				name: '誠心敬拜',
				firstLine: '全心歡欣膝敬拜頌讚你永活神',
				remark: '',
				stroke: 13,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '13-Y0101',
				name: '遇見',
				firstLine: '夜裡漆黑的一片 就算要看看不見',
				remark: 'BY TTK',
				stroke: 13,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '13-Z0101',
				name: '頌讚主',
				firstLine: '頌讚主因祂的愛超出那高天',
				remark: '',
				stroke: 13,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '13-Z0102',
				name: '頌讚全能上帝',
				firstLine: '清風輕吹天空掛著耀眼太陽',
				remark: '',
				stroke: 13,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '13-Z0103',
				name: '頌讚你HALLELUJAH',
				firstLine: '感激基督帶領恩典賜予我未停',
				remark: '',
				stroke: 13,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '13-Z0104',
				name: '頌讚歸於耶穌的聖名',
				firstLine: '頌讚歸於耶穌的聖名',
				remark: '',
				stroke: 13,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '14-F0101',
				name: '福音的大門',
				firstLine: '一條大河波浪寬',
				remark: '',
				stroke: 14,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '14-G0101',
				name: '緊緊抓住昂',
				firstLine: '我無助的時候 昂給我力量',
				remark: '',
				stroke: 14,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '14-J0101',
				name: '與神同行',
				firstLine: '',
				remark: '',
				stroke: 14,
				key: 'Bm',
				link: '',
				lyrics: ''
			},
			{
				code: '14-M0101',
				name: '滿足的喜樂',
				firstLine: '在你面前有滿足,有滿足的喜樂',
				remark: '',
				stroke: 14,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '14-M0102',
				name: '滿有能力',
				firstLine: '',
				remark: '',
				stroke: 14,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '14-N0101',
				name: '寧靜谷',
				firstLine: '在我心靈深處',
				remark: '',
				stroke: 14,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '14-W0101',
				name: '榮耀都歸你',
				firstLine: '哈利路亞 榮耀歸寶座羔羊',
				remark: '',
				stroke: 14,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '14-W0102',
				name: '榮耀榮耀神的羔羊',
				firstLine: '榮耀榮耀神的羔羊',
				remark: '',
				stroke: 14,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '14-W0103',
				name: '榮耀歸於真神',
				firstLine: '榮耀榮耀歸於真神',
				remark: '',
				stroke: 14,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '14-W0104',
				name: '榮耀歸與昂全能的神',
				firstLine: '哦天父我們何等愛昂',
				remark: '',
				stroke: 14,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '14-W0105',
				name: '榮耀耶穌',
				firstLine: '昂是創造天地大主宰',
				remark: '',
				stroke: 14,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '14-Z0101',
				name: '盡心盡性',
				firstLine: '全地齊來頌讚聲遍響宇宙',
				remark: '',
				stroke: 14,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '14-Z0102',
				name: '盡心盡性讚美祂',
				firstLine: '我要盡心盡性來敬拜我主',
				remark: '',
				stroke: 14,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '14-Z0103',
				name: '盡情的敬拜',
				firstLine: '耶穌聖潔公義救主',
				remark: '',
				stroke: 14,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '14-Z0201',
				name: '像當年大衛',
				firstLine: '像當年大衛在神面前',
				remark: '',
				stroke: 14,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '15-C0101',
				name: '齊賀主愛',
				firstLine: '就讓這天一起慶賀',
				remark: '',
				stroke: 15,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '15-C0102',
				name: '齊齊來讚美上主',
				firstLine: '齊齊來讚美上主',
				remark: '',
				stroke: 15,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '15-C0103',
				name: '齊來崇拜歌',
				firstLine: '齊來崇主信徒,快樂又歡欣',
				remark: '',
				stroke: 15,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '15-C0201',
				name: '請差遣我',
				firstLine: '感慨﹗造物與尊貴',
				remark: '',
				stroke: 15,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '15-J0101',
				name: '熱切渴慕神',
				firstLine: '',
				remark: '',
				stroke: 15,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '15-K0101',
				name: '靠著耶穌得勝',
				firstLine: '在你愛裡我要宣告',
				remark: '',
				stroke: 15,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '15-Q0101',
				name: '賜我自由',
				firstLine: '賜自由的靈啊',
				remark: '',
				stroke: 15,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '15-S0101',
				name: '誰曾應許',
				firstLine: '誰曾應許一生不撒下我',
				remark: '',
				stroke: 15,
				key: 'Eb',
				link: '',
				lyrics: ''
			},
			{
				code: '15-S0102',
				name: '誰是主角',
				firstLine: '在這燈飾璀璨都市中',
				remark: '',
				stroke: 15,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '16-C0101',
				name: '親愛的聖靈',
				firstLine: '親愛的聖靈',
				remark: '',
				stroke: 16,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '16-C0102',
				name: '親眼看見昂',
				firstLine: '有你我生命不一樣',
				remark: '',
				stroke: 16,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '16-C0103',
				name: '親愛取穌來',
				firstLine: '耶穌願你降臨, 停留我內心中',
				remark: '',
				stroke: 16,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '16-H0101',
				name: '興起發光',
				firstLine: '心渴望為主發光',
				remark: '',
				stroke: 16,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '16-J0101',
				name: '靜候恩主',
				firstLine: '靜候恩主 萬念敞開',
				remark: '',
				stroke: 16,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '16-J0102',
				name: '靜候昂',
				firstLine: '愁煩苦惱 我心傷痛',
				remark: 'BY TTK',
				stroke: 16,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '16-M0101',
				name: '默默回望',
				firstLine: '望見星星心裡',
				remark: '',
				stroke: 16,
				key: 'Am',
				link: '',
				lyrics: ''
			},
			{
				code: '16-P0101',
				name: '憑藉恩典',
				firstLine: '',
				remark: '',
				stroke: 16,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '16-Y0101',
				name: '擁抱你',
				firstLine: '誰叫風霜 隨意擾攘',
				remark: '',
				stroke: 16,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '17-W0101',
				name: '環繞',
				firstLine: '你問我幫助從何以來 從造天地萬物主而來',
				remark: '',
				stroke: 17,
				key: 'Bm',
				link: '',
				lyrics: ''
			},
			{
				code: '17-Y0101',
				name: '應當一無掛慮',
				firstLine: '應當一無掛慮 拋開心頭的擔',
				remark: '',
				stroke: 17,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '17-Z0101',
				name: '謝謝我主',
				firstLine: '謝謝我主 願施恩治我',
				remark: '',
				stroke: 17,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '18-F0101',
				name: '翻轉地球',
				firstLine: '眼光放的更遠',
				remark: '',
				stroke: 18,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '18-F0201',
				name: '豐盛的應許',
				firstLine: '你仰瞼保守你所愛的',
				remark: '',
				stroke: 18,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '18-G0101',
				name: '舊事已過',
				firstLine: '神是我牧人 神是安慰',
				remark: '',
				stroke: 18,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '18-G0102',
				name: '舊日我失迷',
				firstLine: '舊日我失迷',
				remark: '',
				stroke: 18,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '18-T0101',
				name: '禱告',
				firstLine: '禱告',
				remark: '',
				stroke: 18,
				key: 'E',
				link: '',
				lyrics: ''
			},
			{
				code: '18-Z0101',
				name: '轉回',
				firstLine: '倘若我今天跌倒',
				remark: '',
				stroke: 18,
				key: 'Ab',
				link: '',
				lyrics: ''
			},
			{
				code: '19-Y0101',
				name: '願神興起',
				firstLine: '願神興起 使祂的仇敵四散',
				remark: '',
				stroke: 19,
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: '19-Y0102',
				name: '願昂平安',
				firstLine: '願主看顧您 願平安歸於您',
				remark: '',
				stroke: 19,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '19-Y0103',
				name: '願昂崇高',
				firstLine: '我要在萬民中稱謝昂',
				remark: '',
				stroke: 19,
				key: '',
				link: '',
				lyrics: ''
			},
			{
				code: '19-Y0104',
				name: '願主愛圍繞你',
				firstLine: '願天父上帝的慈愛，常與您們同在',
				remark: '',
				stroke: 19,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '20-B0101',
				name: '寶貴十架',
				firstLine: '主耶穌 我感謝昂',
				remark: '',
				stroke: 20,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '20-H0101',
				name: '獻上活祭',
				firstLine: '我心切切渴慕昂主 我要緊緊跟隨你',
				remark: '',
				stroke: 20,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '20-H0102',
				name: '獻給我天上的主',
				firstLine: '在我心中有千百萬心聲',
				remark: '',
				stroke: 20,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '21-S0101',
				name: '屬靈人',
				firstLine: '',
				remark: '',
				stroke: 21,
				key: 'Dm',
				link: '',
				lyrics: ''
			},
			{
				code: '22-F0101',
				name: '歡呼主基督',
				firstLine: '歡呼主基督 歡呼以馬內利',
				remark: '',
				stroke: 22,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '22-F0102',
				name: '歡欣',
				firstLine: '歡欣 心裡感謝神',
				remark: '',
				stroke: 22,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '22-T0101',
				name: '聽啊以色列',
				firstLine: '聽啊以色列',
				remark: '',
				stroke: 22,
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: '22-T0102',
				name: '聽啊天軍讚美聲',
				firstLine: '按照聖經裡所訓示',
				remark: '',
				stroke: 22,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '24-Y0101',
				name: '讓我高飛',
				firstLine: '主讓我心夢想',
				remark: '',
				stroke: 24,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '24-Y0102',
				name: '讓我靈自由',
				firstLine: '讓我靈自由 使我能敬拜昂',
				remark: '',
				stroke: 24,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '24-Y0103',
				name: '讓讚美飛揚',
				firstLine: '在這美麗的時刻',
				remark: '',
				stroke: 24,
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: '24-Y0104',
				name: '讓昂大能改變我',
				firstLine: '願聖靈此刻充滿我',
				remark: '',
				stroke: 24,
				key: 'Am',
				link: '',
				lyrics: ''
			},
			{
				code: '24-Y0105',
				name: '讓我親近昂',
				firstLine: '渴望能遇見你，渴望能親近你',
				remark: '',
				stroke: 24,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '24-Y0106',
				name: '讓我帶著讚頌作祭物',
				firstLine: '讓我帶著讚頌作祭物',
				remark: '',
				stroke: 24,
				key: 'A',
				link: '',
				lyrics: ''
			},
			{
				code: '24-Y0107',
				name: '讓我得見昂的榮面',
				firstLine: '我的靈安靜在昂面前',
				remark: '',
				stroke: 24,
				key: 'Bb',
				link: '',
				lyrics: ''
			},
			{
				code: '24-Y0108',
				name: '讓我',
				firstLine: '昂用重價救贖我',
				remark: '',
				stroke: 24,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '26-Z0101',
				name: '讚美之泉',
				firstLine: '從天父而來的愛和恩典',
				remark: '',
				stroke: 26,
				key: 'D',
				link: '',
				lyrics: ''
			},
			{
				code: '26-Z0102',
				name: '讚美耶和華',
				firstLine: '萬國阿 你們都當讚美耶和華',
				remark: '',
				stroke: 26,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: '26-Z0103',
				name: '讚頌昂的愛',
				firstLine: '願意讚頌昂的愛',
				remark: '',
				stroke: 26,
				key: 'F',
				link: '',
				lyrics: ''
			},
			{
				code: 'E-G',
				name: 'Glorify the Name',
				firstLine: '',
				remark: '',
				stroke: '',
				key: 'C',
				link: '',
				lyrics: ''
			},
			{
				code: 'E-H',
				name: 'Hallelujah 讚美主',
				firstLine: 'Hallelujahl 來同聲歡唱',
				remark: '',
				stroke: '',
				key: 'G',
				link: '',
				lyrics: ''
			},
			{
				code: 'E-J',
				name: 'Jehorah-Jireh',
				firstLine: 'Jehorah-Jireh (耶和華以勒)',
				remark: '',
				stroke: '',
				key: 'Em',
				link: '',
				lyrics: ''
			},
			{
				code: 'E-S',
				name: 'Shalom 平安',
				firstLine: '沙龍 沙龍祂名是耶和華沙龍',
				remark: '',
				stroke: '',
				key: 'F',
				link: '',
				lyrics: ''
			},
		],
	},
	methods: {
		search(searchTerms) {
			this.filteredSongs = []
			for (i = 0; i < this.songs.length; i++) {
				if (searchTerms.searchCode && searchTerms.searchCode != this.songs[i].code) {
					// searchCode not matched
					continue
				}
				
				if (searchTerms.stroke && searchTerms.stroke != this.songs[i].stroke) {
					// stroke not matched
					continue
				}
				
				if (searchTerms.nameLength && searchTerms.nameLength != this.songs[i].name.length) {
					// name length not matched
					continue
				}
				
				if (searchTerms.wordContains) {
					var searchWord = searchTerms.wordContains.toLowerCase()
					if (!this.songs[i].name.toLowerCase().includes(searchWord) && !this.songs[i].firstLine.toLowerCase().includes(searchWord) && !this.songs[i].remark.toLowerCase().includes(searchWord)) {
						// wordContains not matched
						continue
					}
				}
				
				if (searchTerms.key && searchTerms.key != this.songs[i].key) {
					// key not matched
					continue
				}
				
				this.filteredSongs.push(this.songs[i])
			}
		},
		scrollToTop() {
			window.scrollTo(0,0);
		},
	},
})